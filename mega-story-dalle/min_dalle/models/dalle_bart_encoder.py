...
class DalleBartEncoder(nn.Module):
    ...
    def resize_token_embeddings(self, new_num_tokens):
        ...
        new_embeddings.weight.data[:n, :] = self.embed_tokens.weight.data[:n, :]
    ...
    def forward(self, text_tokens: LongTensor) -> FloatTensor:
        ...
        encoder_state = (
            self.embed_tokens.forward(text_tokens) +
            self.embed_positions.forward(torch.stack([torch.arange(L, device=text_tokens.device)]*B))
        )
        # print(encoder_state.requires_grad, encoder_state.grad_fn)
