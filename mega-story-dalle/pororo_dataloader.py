...
class StoryImageDataset(torch.utils.data.Dataset):
    def __init__(self, img_folder, tokenizer, preprocess, mode='train', video_len=4, out_img_folder='', return_labels=False, max_length=64):
        self.lengths = []
        self.followings = []
        self.images = []
        self.img_dataset = ImageFolder(img_folder)
    ...
    def __getitem__(self, item):
        ...
        if self.tokenizer is not None:
            tokens = [self.tokenizer.tokenize(caption.lower())[:self.max_length] for caption in captions]

            if False:
                # superconditioning during generation
                text_tokens = np.ones((2 * len(tokens), self.max_length), dtype=np.int32)
                ...
                text_tokens = np.ones(( len(tokens), self.max_length), dtype=np.int32)
                for i, words in enumerate(tokens):
